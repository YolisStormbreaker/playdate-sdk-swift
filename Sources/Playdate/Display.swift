...
extension Display {
  ...
  /// and does not account for lag due to (for example) 
  /// code running too slow. To get the real time 
  /// frame rate, use ``Display.fps``.
  public static var refreshRate: Float {
    displayAPI.getRefreshRate.unsafelyUnwrapped()
  }
  
  /// Returns the measured, actual refresh rate in frames per second. 
  /// This value may be different from the specified refresh rate 
  /// (see ``Display.refreshRate``) by a little or a lot 
  /// depending upon how much calculation is being done per frame.
  public static var fps: Float {
    displayAPI.getFPS.unsafelyUnwrapped()
  }  
